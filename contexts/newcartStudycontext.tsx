...
interface Product {
    id: number,
    price: number,
    img: any, // Changed from StaticImageData to any for compatibility
    title: string,
    categories: string,
    novelties: boolean,
    discounted: boolean
...
const cart_reducer = (state: CartState, action: CartAction): CartState => {
    ...
    switch (action.type) {
        case CART_ACTION.ADD_ITEM:
            // TypeScript now knows action.payload is Product
            if (!foundProduct) {
                const newState = {
        ...
        case CART_ACTION.LOAD_FROM_STORAGE:
            // TypeScript now knows action.payload is CartItem[]
