...
const cart_reducer = (state: CartState, action: CartAction): CartState => {
    ...
    switch (action.type) {
        case CART_ACTION.ADD_ITEM:
            if (!foundProduct) {
                const newState = {
                    products: [...state.products, { product: action.payload as Product, quantity: 1 }]
                };
                ...
                const otherProducts = state.products.filter(item => item.product.id != (action.payload as Product).id);

                const newState = {
                    products: [...otherProducts, { product: action.payload as Product, quantity: foundProduct.quantity + 1 }]
                };
